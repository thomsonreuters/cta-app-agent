...
class JobQueue extends PriorityQueue {
   ...
   */
  has(id) {
    return this.priv.data.some((job) => job.id === id);
  }
   ...
   */
  remove(id) {
    return _.remove(this.priv.data, (job) => job.id === id)[0];
  }
  ...
  queue(job) {
    ...
    if (this.has(job.id)) {
      throw new Error(`A job with id ${job.id} is already present in queue.`);
    } else {
      super.queue(job);
    }
    return this;
