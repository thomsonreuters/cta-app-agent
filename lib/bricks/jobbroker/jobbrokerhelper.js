...
class JobBrokerHelper {
      ...
      case 'state-create':
        {
          ...
          this.logger.info(`Execution: ${job.payload.executionId} (STATE) - Status: ${stateJob.payload.status} - sent`);
          break;
        }
  ...
  createContextForExecutionRun(job, options) {
    ...
    const context = that.cementHelper.createContext(execJob, ['canceled'])
  ...
  setPendingTimeout(job) {
    const jobid = job.payload.execution.id;
    const timeout = job.payload.execution.pendingTimeout;
