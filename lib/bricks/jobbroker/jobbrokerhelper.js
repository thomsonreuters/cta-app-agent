...
class JobBrokerHelper {
  ...
  ack(job) {
    ...
    const groupjob = this.runningJobs.read.get(job.payload.groupExecutionId);
   ...
   * @returns {Context} the created context
   */
  createContextForExecutionRun(job) {
    const that = this;
    const jobId = job.payload.execution.id;
    ...
    context.once('accept', function onContextAccept(who) {
      that.send({
        ...
        payload: {
          executionId: jobId,
          status: 'running',
          message: `Job ${jobId} accepted by ${who}.`,
        },
      });
      that.ack(job);
    })
      ...
      .once('done', function onContextDone(who, res) {
        ...
        that.remove(job);
        if (that.runningJobs.read.has(job.payload.groupExecutionId)) {
          if (res.hasOwnProperty('cancelMode') && res.cancelMode === 'MANUAL') {
            that.terminateGroupJob(job.payload.groupExecutionId, {
              ...
              payload: {
                ...
                status: 'canceled',
                message: `group Job ${job.payload.groupExecutionId} canceled (MANUAL) during sub-Job ${jobId}`,
              },
            });
          } else {
            const groupJob = that.runningJobs.read.get(job.payload.groupExecutionId);
            that.send({
              nature: {
                type: 'message',
                quality: 'get',
  ...
  processDefault(job) {
    ...
    if (!noRunningJobs) {
      if (job.payload.execution.priority === 0
        || this.runningJobs.read.has(job.payload.groupExecutionId)) {
        this.send(job);
      } else {
        try {
  ...
  cancel(cancelationJob) {
    if (this.runningJobs.read.has(cancelationJob.payload.execution.id)) {
      const jobToCancel = this.runningJobs.read.get(cancelationJob.payload.execution.id);
  ...
  cancelGroupJob(cancelationJob, groupJob) {
    const that = this;
    this.runningJobs.cancel.set(cancelationJob.payload.execution.id, cancelationJob);
    // this.send({
    //   nature: {
    //     type: 'state',
    //     quality: 'create',
    ...
    //   },
    // });
    const runningSubJobs = Array.from(this.runningJobs.run.values())
      .filter((runningJob) => (runningJob.payload.groupExecutionId === groupJob.payload.execution.id));
    if (runningSubJobs.length > 0) {
      runningSubJobs.forEach((subJob) => {
        const cancelSubJob = {
          ...
          payload: {
            execution: {
              id: subJob.payload.execution.id,
            },
            //groupExecutionId: groupJob.payload.execution.id,
          },
        };
