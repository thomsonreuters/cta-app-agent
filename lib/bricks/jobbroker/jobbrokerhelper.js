...
class JobBrokerHelper {
  ...
  createContextForExecutionRun(job, options) {
    ...
    const setRunningJobForResult = {
      ...
      payload: {
        ...
        testName: job.payload.testSuite.tests[testIndex].name,
        testDescription: job.payload.testSuite.tests[testIndex].description,
      },
    };
  ...
  setPendingTimeout(job) {
    ...
    job.pendingTimeout = setTimeout(() => { // eslint-disable-line no-param-reassign
  ...
  setRunningTimeout(job) {
    ...
    job.timeout = setTimeout(() => { // eslint-disable-line no-param-reassign
  ...
  cancelQueuedJob(cancelationJob) {
    ...
    if (dequeued) {
      ...
      this.logger.warn(`tried to cancel an unknown job: ${cancelationJob.payload.execution.id}`);
      this.ack(cancelationJob);
    }
  }
}
