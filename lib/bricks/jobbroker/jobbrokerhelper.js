...
class JobBrokerHelper {
  ...
  createContext(job, options) {
    ...
    switch (typeAndQuality) {
      ...
      case 'execution-acknowledge':
      case 'execution-changestate':
      default:
        {
  ...
  createContextForCommandline(job) {
    ...
    context = that.cementHelper.createContext(job);
  ...
  createContextForGroup(job) {
    ...
    context = that.createContextForQueueGet({
      'nature': {
        'type': 'message',
        'quality': 'get',
      },
  ...
  /**
   * Terminate a group Job (ack, send changestate, remove)
   * @param {ObjectId|String} groupjobid - the id of the group Job  to terminate
   * @param {Job} [changestate] - an optional execution-changestate to send
   * @private
   */
  terminateGroupJob(groupjobid, changestate) {
    if (this.runningJobs.has(groupjobid)) {
      const groupjob = this.runningJobs.get(groupjobid);
      this.ack(groupjob);
      if (changestate) {
        this.send(changestate);
      }
  ...
  cancel(cancelationJob) {
    if (this.runningJobs.has(cancelationJob.payload.jobid)) {
      ...
      const typeAndQuality = (jobToCancel.nature.type + '-' + jobToCancel.nature.quality).toLowerCase();
      switch (typeAndQuality) {
        case 'execution-group': {
