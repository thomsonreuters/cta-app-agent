...
class JobHandlerHelper {
  ...
  getExecutor(job) {
    let executor;
    if (job.nature.quality.trim().toLowerCase() === 'cancelation') {
      ...
      if (this.runningJobs.has(job.payload.jobid)) {
        ...
        executor = this.executors.get(jobToCancel.nature.quality.trim().toLowerCase());
      }
    } else {
      executor = this.executors.get(job.nature.quality.trim().toLowerCase());
    }
    return executor;
  }
