 ...
 * @property {Number} DEFAULTS.CMDTIMEOUT - The default timeout used to cancel a Command/Stage of a Job after expiration
 */
class CommandLine extends Executor {
  ...
  constructor(defaults, logger) {
    ...
    this.CANCELMODE.STAGETIMEOUT = 'STAGE TIMEOUT';
  ...
  validate(job) {
    return new Promise((resolve, reject) => {
      ...
      co(function* validateCoroutine() {
        ...
        for (let index = 0; index < job.payload.stages.length; index++) {
          ...
          if ('env'in stage) {
            if (!(Array.isArray(stage.env))) {
              ...
              for (let envIndex = 0; envIndex < stage.env.length; envIndex++) {
  ...
  _createFile(content) {
    return new Promise((resolve, reject) => {
      ...
      const filepath = path.join(tmpDir, new Date().getTime() + Math.random().toString(36).slice(2) + fileExtension);
  ...
  _execFile(filepath, env, cwd) {
    return new Promise((resolve, reject) => {
      ...
      try {
        const spawn = cp.spawn(filepath, [], options);
  ...
  _executeStage(jobid, stageidx, stop, callback) {
    ...
    return co(function* executeStage() {
      ...
      process.on('close', function onProcessClosed(code) {
        ...
        if (that.runningJobs[jobid].stageTimeout) clearTimeout(that.runningJobs[jobid].stageTimeout);
        if (stop) {
          clearTimeout(that.runningJobs[jobid].stopTimeout);
          if (that.runningJobs[jobid].canceled) {
            delete that.runningJobs[jobid];
          }
        }
      ...
      co(function* execute() {
        ...
        function onStageFinished(process, code) {
          if (!that.runningJobs.hasOwnProperty(job.id) || that.runningJobs[job.id].canceled) {
            ...
            clearTimeout(that.runningJobs[job.id].timeout);
        ...
        if (that.runningJobs.hasOwnProperty(job.id)) {
          if (that.runningJobs[job.id].timeout) clearTimeout(that.runningJobs[job.id].timeout);
          delete that.runningJobs[job.id];
        }
        if (callback && typeof callback === 'function') {
          callback(error, {
  ...
  _cancel(jobid, mode, callback) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!this.runningJobs.hasOwnProperty(jobid)) {
        ...
        clearTimeout(this.runningJobs[jobid].timeout);
        this.logger.info(`Sending SIGKILL to process ID ${process.pid}`);
        this._kill(process.pid, 'SIGKILL', (err) => {
          if (err) {
            that.logger.error('Sending SIGKILL failed with: ' + err);
            response.message = `Process killing failed with: ${err}`;
            response.error = err;
          }
