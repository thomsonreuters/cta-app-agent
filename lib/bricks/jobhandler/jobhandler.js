...
class JobHandler extends Brick {
  ...
  validate(context) {
    ...
    return co(function* validateCoroutine() {
      ...
      if (!that.executors.has(quality) && quality !== 'cancel') {
  ...
  process(context) {
    const that = this;
    const job = context.data;
    if (job.nature.quality === 'cancel' && !this.runningJobs.has(job.payload.jobid)) {
      ...
      that.jobHandlerHelper.runExecutor(executor, job, function onJobFinished(err, finishResponse) {
        if (err) {
          ...
          switch (finishResponse.cancelMode) {
            case 'manual':
              context.emit('canceled', that.name, finishResponse);
              break;
            case 'executionTimeout':
            default:
              context.emit('timeout', that.name, finishResponse);
              break;
          }
        } else {
