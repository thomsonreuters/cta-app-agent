...
const executors = {
  'CommandLine': require('./executor/commandline'),
};
...
class JobHandler extends Brick {
  ...
  constructor(cementHelper, config) {
    ...
    this.executors.set('commandline', new executors.CommandLine(config.properties.COMMANDLINE, this.logger));
  ...
  process(context) {
    const that = this;
    const job = context.data;
    if (job.nature.quality.trim().toLowerCase() === 'cancelation' && !this.runningJobs.has(job.payload.jobid)) {
